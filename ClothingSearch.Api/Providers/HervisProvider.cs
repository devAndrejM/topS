...
namespace ClothingSearch.Api.Providers
{
    public class HervisProvider : IStoreProvider
    {
        ...
        public HervisProvider(HttpClient httpClient)
        {
        ...
        {
            return countryId == 1; // Croatia only
        }

        public async Task<List<ProductDto>> SearchAsync(string query, UserSetting userSettings, string category = "")
        {
            await Task.Delay(2000); // Simulate scraping delay

            // Use actual product images from unsplash or real store images
            return new List<ProductDto>
            {
                ...
                new ProductDto
                {
                    ...
                    Price = 199.00m,
                    Currency = "HRK",
                    ImageUrl = "https://images.unsplash.com/photo-1506629905607-d405d7a94c9a?w=300&h=300&fit=crop",
